...
import { Pill } from "@/app/Drag/ScrollablePills";
import { Badge } from "@/components/ui/badge";
import DraggableSpace from "@/app/Drag/DraggableSpace";
...
const SortablePill = ({
  ...
  item: Pill & { calculatedWidth: number };
}) => {
    ...
    >
        ...
        <SortableContext items={items.map((item) => item.id)}>
          {items.map((item, index) => (
            <React.Fragment key={item.id}>
              ...
              {overIndex === index + 1 && activeId !== item.id && (
                <DropLineIndicator />
              )}
            </React.Fragment>
          ))}
      ...
      {isMounted && activeId && (
        <DragOverlay>
          <Badge
            style={{
              width: items.find((item) => item.id === activeId)
                ?.calculatedWidth,
            }}
            className="px-2 py-1 rounded-md text-white cursor-grab flex items-center justify-center transition-colors whitespace-nowrap"
          >
